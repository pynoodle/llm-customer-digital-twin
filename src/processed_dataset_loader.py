...
class ProcessedDatasetLoader:
    ...
    def load(self) -> None:
        """전처리된 CSV 파일을 로드합니다."""
        print(f"📦 전처리된 데이터셋 로딩 중: {self.csv_path}")
        
        if not os.path.exists(self.csv_path):
            print(f"❌ 파일을 찾을 수 없습니다: {self.csv_path}")
            print("💡 먼저 create_processed_dataset.py를 실행하여 데이터를 전처리하세요.")
            return
    ...
    def _create_personas(self) -> None:
        ...
        print(f"✅ {len(self.personas)}개 페르소나 생성 완료")
...
def main():
    """테스트 함수"""
    print("🧪 ProcessedDatasetLoader 테스트")
