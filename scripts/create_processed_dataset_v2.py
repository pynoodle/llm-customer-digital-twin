"""
Twin-2K-500 데이터셋 전처리 스크립트 v2
persona_json 파싱 문제를 해결하여 정리된 데이터셋을 생성합니다.
"""
...
def create_processed_dataset_v2():
    """전처리된 데이터셋을 생성합니다."""
    print("🚀 Twin-2K-500 데이터셋 전처리 v2 시작...")
    ...
    for idx, row in df.iterrows():
        ...
        if persona_json:
            try:
                ...
                # 이미 딕셔너리인 경우
                elif isinstance(persona_json, dict):
                    json_data = persona_json
                # 리스트인 경우 (일부 데이터가 리스트 형태)
                elif isinstance(persona_json, list):
                    # 리스트의 첫 번째 요소가 딕셔너리인 경우
                    if len(persona_json) > 0 and isinstance(persona_json[0], dict):
                        json_data = persona_json[0]
                    else:
                        json_data = {}
                else:
                    json_data = {}
    ...
    print(f"✅ 파싱 완료: 성공 {success_count}개, 실패 {fail_count}개")
    ...
    # CSV 저장
    csv_path = os.path.join(output_dir, "twin2k500_processed_v2.csv")
    processed_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    print(f"\n💾 CSV 저장 완료: {csv_path}")
    
    # Excel 저장 (샘플 100개만)
    excel_path = os.path.join(output_dir, "twin2k500_sample_v2.xlsx")
    ...
    stats = {
        ...
        'parsing_fail': fail_count
    }
    
    # 통계 JSON 저장
    stats_path = os.path.join(output_dir, "dataset_stats_v2.json")
    ...
    print(f"📄 메인 파일: twin2k500_processed_v2.csv")
    
    return processed_df, stats

if __name__ == "__main__":
    try:
        processed_df, stats = create_processed_dataset_v2()
        print(f"\n✅ 성공적으로 완료되었습니다!")
    except Exception as e:
        print(f"\n❌ 오류 발생: {e}")
        import traceback
